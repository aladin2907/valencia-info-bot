...
Боевой стек живёт в AWS ECS (регион `us-east-1`), инфраструктуру ведёт devops-команда
через Terraform: три сервиса `models`, `api`, `bot` на одном узле, база RDS Postgres,
секреты в Secrets Manager, модель LLM зашита в описание задачи. Мы меняем образы и
настройки приложения, инфраструктуру — через devops. Известные ограничения:

- задача `bot` не помещается на узел по числу сетевых интерфейсов (`RESOURCE:ENI`) —
  нужен ENI-trunking, второй узел или другой сетевой режим;
- база включается планировщиком только в рабочие часы по будням; включить вручную
  можно вызовом Lambda `instance-scheduler` с `{"action":"start"}`;
- после перехода на OpenAI напрямую в описании задачи должны быть `LLM_BASE_URL` и
  `LLM_MODEL`, а ключ — в секрете.

Ниже — ручной путь на одном EC2 с Supabase. Он проверен по частям (схема, поиск,
образы), но боевой стек собран не по нему.
...
Считать векторы на процессоре небыстро: у нас в Docker вышло 0,2–0,8 треда в
секунду, то есть все 20 633 треда — это ночь работы. Сначала прогони одну
группу с `--limit 500`, убедись что ответы нормальные, потом запускай остальное
на ночь. Прогон можно прерывать: посчитанное сохраняется, повтор продолжает с
того же места.
...
Пока не заполнены `TG_API_ID` / `TG_API_HASH`, прогон падает на первом же шаге —
это специально, тихо пропускать докачку он не будет. Если данные заливаются из
файлов, а не из Telegram, добавь `--skip-fetch`:
`docker compose run --rm ingest python -m ingest.nightly --skip-fetch`.

## 7. Бот

Токен `@valencia_info_bot` сейчас занят: обновления уходят на webhook
n8n (`n8n.mafiavlc.org`). Пока это так, наш бот **не запустится** — он проверяет
webhook при старте и отказывается работать, чтобы не сломать живого бота.
Одновременно два бота на одном токене не живут: либо webhook у n8n, либо опрос
у нас.

Порядок переключения, строго в этом порядке:

1. Залить данные и убедиться, что `POST /ask` отвечает нормально (§5).
2. Выключить workflow в n8n — именно выключить, не просто перестать слушать.
3. Запустить бота с разрешением снять webhook:
   `ALLOW_WEBHOOK_TAKEOVER=1 docker compose up -d bot`
   (флаг нужен один раз; после снятия webhook бот стартует и без него).
4. Проверить в Telegram живым вопросом.

Откат: остановить `bot`, включить workflow в n8n обратно — он поставит свой
webhook сам.

Токен светился в переписке, так что после переключения его стоит перевыпустить
в `@BotFather` (`/revoke`) и обновить в `.env` на сервере.
